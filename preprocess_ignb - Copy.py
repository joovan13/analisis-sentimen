...
def sentiment_analyze_nb(data):
    ...
    model, vectorizer = train_model_nb(X_train, y_train)
...
def sentiment_analyze_ignb(data, gain_df, threshold):
    ...
    model = train_model_ignb(X_train_selected, y_train)
        ...
        if st.button('Analyze'):
            if 'preprocessed_data' in st.session_state:
                ...
                if analysis_choice == 'Naive Bayes Classifier':
                    accuracy, report, conf_matrix, precision, recall, f1 = sentiment_analyze_nb(
                        data)
                    st.write(f"**Accuracy:** {accuracy}")
                    st.write("**Confusion Matrix:**")
                    st.write(conf_matrix)
